...
datos <- read_excel("bd_2024-1/3. Formulario preselección de cursos intercambio 2024-2. (1-13)_REZAGADOS.xlsx",
                    sheet = "Respuestas formulario 3")
...
write_xlsx(cursos_desagregados, "resultados/formulario_rezagados1.xlsx")
...
deportivos <- deportivos %>% 
  ...
  rename(C0NNS =`Curso deportivo 1. Número de la sección`) %>% 
  rename(C1NNS =`Curso deportivo 2. Número de la sección`) %>%
  rename(C2NNS =`Curso deportivo 3. Número de la sección`) %>% 
...
write_xlsx(deportivos_enumerados, "resultados/formulario_rezagados1dep.xlsx")
